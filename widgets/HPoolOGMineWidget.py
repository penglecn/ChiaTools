...
from ui.HPoolOGMineWidget import Ui_HPoolOGMineWidget
...
class HPoolOGMineWidget(QWidget, Ui_HPoolOGMineWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if 'hpool_miner_name' in config:
            self.editMinerName.setText(config['hpool_miner_name'])
        else:
            self.editMinerName.setText(socket.gethostname())
            self.saveMineConfig()
        self.editMinerName.editingFinished.connect(self.saveMineConfig)

        if 'hpool_apikey' in config:
            self.editApiKey.setText(config['hpool_apikey'])
    ...
    def setMainWindow(self, win):
        ...
        if 'hpool_auto_mine' not in config:
            auto_start = is_auto_launch()
        else:
            auto_start = config['hpool_auto_mine']
    ...
    def mineTerminated(self):
        ...
        index = self.main_window.tabWidget.indexOf(self.main_window.tabHPoolOGMine)
        self.main_window.tabWidget.setTabText(index, 'HPoolOG老矿池挖矿')
    ...
    def saveMineConfig(self):
        ...
        config['hpool_miner_name'] = miner_name
        config['hpool_apikey'] = apikey
    ...
    def startMine(self, manual=False):
        ...
        miner_name = config['hpool_miner_name']
        apikey = config['hpool_apikey']
        ...
        index = self.main_window.tabWidget.indexOf(self.main_window.tabHPoolOGMine)
        self.main_window.tabWidget.setTabText(index, 'HPoolOG老矿池挖矿（正在挖矿）')
...
path:
{paths}
minerName: {config['hpool_miner_name']}
apiKey: {config['hpool_apikey']}
